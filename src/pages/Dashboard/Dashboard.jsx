...
export const Dashboard = ({ setActiveTab }) => {
  ...
  // Start everything empty for a new user
  ...
  const healthScore = () => {
    const hasTrackedActivity =
      waterGlasses > 0 ||
      medications.some((med) => med.takenToday) ||
      latestSymptom !== null;

    // New user has not entered anything yet
    ...
    // Hydration = maximum 30 points
    if (waterGlasses > 0) {
      score += Math.round((waterGlasses / 8) * 30);
    }

    // Medication adherence = maximum 40 points
    if (medications.length > 0) {
      ...
      score += Math.round(
        (takenMedications / medications.length) * 40
      );
    }

    // Logging symptoms = 30 points
    if (latestSymptom) {
      score += 30;
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-6 rounded-3xl glass-card flex flex-col items-center justify-center text-center shadow-pastel">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
            ...
            {currentHealthScore === null
              ? 'Start tracking your health to generate your score.'
              : 'Your score is based on today’s tracked activities.'}
